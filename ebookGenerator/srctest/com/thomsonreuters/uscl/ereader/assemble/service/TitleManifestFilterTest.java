...
public class TitleManifestFilterTest extends TitleMetadataTestBase {
	...
	private static final String EXPECTED_DOCS = EXPECTED_DOCS_TOC_ENTRIES
			+ "<doc id=\"DOC_GUID\" src=\"DOC_GUID.html\"/></docs>";
	...
	public void setUp() throws Exception {
		...
		titleManifestFilter = new TitleManifestFilter(titleMetadata, new HashMap<String, String>(), uuidGenerator,
				temporaryDirectory, mockFileUtilsFacade, mockPlaceholderDocumentService, altIdMap);
	...
	public void testWriteAssets() throws Exception {
		...
		Assert.assertEquals(EXPECTED_ASSETS + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
	...
	public void testWriteAuthors() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_AUTHORS + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteKeywords() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_KEYWORDS + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteCopyright() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_COPYRIGHT + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testStartManifest() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(
				EXPECTED_START_MANIFEST_PREFIX + lastupdated + EXPECTED_START_MANIFEST_SUFFIX_SINGLETON
						+ onlineexpiration + "\">" + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteDisplayName() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_DISPLAYNAME + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteFeatures() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_FEATURES + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteMaterialId() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_MATERIAL_ID + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testWriteCoverArt() throws Exception {
		...
		titleManifestFilter.endDocument();
		Assert.assertEquals(EXPECTED_ARTWORK + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}
	...
	public void testStartDocument() throws Exception {
		...
		Assert.assertEquals(EXPECTED_START_MANIFEST_PREFIX + lastupdated + EXPECTED_START_MANIFEST_SUFFIX
				+ onlineexpiration + "\">" + EXPECTED_FEATURES + EXPECTED_MATERIAL_ID + EXPECTED_ARTWORK
				+ EXPECTED_ASSETS + EXPECTED_DISPLAYNAME + EXPECTED_AUTHORS + EXPECTED_KEYWORDS + EXPECTED_COPYRIGHT
				+ EXPECTED_ISBN + EXPECTED_END_MANIFEST, resultStreamToString(resultStream));
	}
	...
	public void testEndDocumentWithParsingToc() throws Exception {
		...
		Assert.assertEquals(
				EXPECTED_START_MANIFEST_PREFIX + lastupdated + EXPECTED_START_MANIFEST_SUFFIX + onlineexpiration + "\">"
						+ EXPECTED_FEATURES + EXPECTED_MATERIAL_ID + EXPECTED_ARTWORK + EXPECTED_ASSETS
						+ EXPECTED_DISPLAYNAME + EXPECTED_AUTHORS + EXPECTED_KEYWORDS + EXPECTED_COPYRIGHT
						+ EXPECTED_TOC + CASCADED_TOC + EXPECTED_DOCS + EXPECTED_ISBN + EXPECTED_END_MANIFEST,
				resultStreamToString(resultStream));
	}

	@Test
	public void testTitleManifestFilterHappyPath() throws Exception {
		InputStream immigrationProceduresHandbook = TitleManifestFilterTest.class
				.getResourceAsStream("IMPH_L1_EXTENSIONS.xml");
		...
		// IOUtils.copy(new ByteArrayInputStream(resultStream.toByteArray()),
		// new FileOutputStream(new
		// File("C:\\Users\\u0081674\\TitleManifestFilterTestOutput.xml")));
		InputSource expected = new InputSource(
				TitleManifestFilterTest.class.getResourceAsStream("IMPH_L1_EXTENSIONS_EXPECTED_MANIFEST.xml"));
		...
		Assert.assertTrue(differences.size() == 1); // the only thing that
													// should be different
													// between the control file
													// and this run is the last
													// updated date.
	...
	public void testFamilyGuidReplacementHappyPath() throws Exception {
		...
		String expected = EXPECTED_START_MANIFEST_PREFIX + lastupdated + EXPECTED_START_MANIFEST_SUFFIX
				+ onlineexpiration + "\">" + EXPECTED_FEATURES + EXPECTED_MATERIAL_ID + EXPECTED_ARTWORK
				+ EXPECTED_ASSETS + EXPECTED_DISPLAYNAME + EXPECTED_AUTHORS + EXPECTED_KEYWORDS + EXPECTED_COPYRIGHT
				+ expectedToc + expectedDocs + EXPECTED_ISBN + EXPECTED_END_MANIFEST;
	...
	public void testDuplicateDocGuidHappyPath() throws Exception {
		...
		String expectedDocs = EXPECTED_DOCS_TOC_ENTRIES
				+ "<doc id=\"FAM_GUID1\" src=\"DOC_GUID1.html\"/><doc id=\"GENERATED_DOC_GUID\" src=\"GENERATED_DOC_GUID.html\"/><doc id=\"FAM_GUID3\" src=\"DOC_GUID3.html\"/></docs>";
		String expectedToc = EXPECTED_TOC
		...
		mockFileUtilsFacade.copyFile(new File(temporaryDirectory.getAbsolutePath() + "\\DOC_GUID1.html"),
	...
	public void testDuplicateFamilyGuidHappyPath() throws Exception {
		...
		String expectedDocs = EXPECTED_DOCS_TOC_ENTRIES
				+ "<doc id=\"FAM_GUID1\" src=\"DOC_GUID1.html\"/><doc id=\"FAM_GUID2\" src=\"DOC_GUID2.html\"/><doc id=\"GENERATED_FAMILY_GUID\" src=\"GENERATED_FAMILY_GUID.html\"/></docs>";
		String expectedToc = EXPECTED_TOC
		...
		UuidGenerator mockUuidGenerator = EasyMock.createMock(UuidGenerator.class);
		mockFileUtilsFacade.copyFile(new File(temporaryDirectory.getAbsolutePath() + "\\DOC_GUID3.html"),
	...
	public void testDuplicateDocGuidAndFamilyGuidHappyPath() throws Exception {
		...
		String expectedDocs = EXPECTED_DOCS_TOC_ENTRIES
				+ "<doc id=\"FAM_GUID1\" src=\"DOC_GUID1.html\"/><doc id=\"FAM_GUID2\" src=\"DOC_GUID2.html\"/><doc id=\"GENERATED_FAMILY_GUID\" src=\"GENERATED_FAMILY_GUID.html\"/><doc id=\"GENERATED_DOC_GUID\" src=\"GENERATED_DOC_GUID.html\"/></docs>";
		String expectedToc = EXPECTED_TOC
	...
	public void testCascadeAcrossSiblingsWorksProperlyCaFedRules2012() throws Exception {
		...
		InputSource result = new InputSource(new ByteArrayInputStream(resultStream.toByteArray()));
		InputSource expected = new InputSource(
				TitleManifestFilterTest.class.getResourceAsStream("CA_FED_RULES_2012_EXPECTED_MANIFEST.xml"));
		DetailedDiff diff = new DetailedDiff(compareXML(expected, result));
		System.out.println(diff.getAllDifferences());
		List<Difference> differences = diff.getAllDifferences();
		Assert.assertTrue(differences.size() == 1); // the only thing that
	...
	public void testCaFedRules2012WithoutDeletedNodes() throws Exception {
		InputStream caFedRules2012 = TitleManifestFilterTest.class
				.getResourceAsStream("CA_FED_RULES_2012_WITHOUT_DELETED_NODES.xml");
		titleManifestFilter.parse(new InputSource(caFedRules2012));
		System.out.println(resultStreamToString(resultStream));
		InputSource result = new InputSource(new ByteArrayInputStream(resultStream.toByteArray()));
		InputSource expected = new InputSource(TitleManifestFilterTest.class
				.getResourceAsStream("CA_FED_RULES_2012_EXPECTED_MANIFEST_WITHOUT_DELETED_NODES.xml"));
		DetailedDiff diff = new DetailedDiff(compareXML(expected, result));
		System.out.println(diff.getAllDifferences());
		List<Difference> differences = diff.getAllDifferences();
		Assert.assertTrue(differences.size() == 1); // the only thing that
	...
	public void testWriteTocNodeHappyPath() throws Exception {
		...
		titleManifestFilter.endDocument(); // indirectly tests
											// writeTableOfContents, which is
											// called as part of invoking
											// endDocument().
		System.out.println(resultStreamToString(resultStream));
		String expected = "<toc><entry s=\"DOC1/TOC1\"><text>FOO</text></entry><entry s=\"DOC2/TOC2\"><text>BAR</text></entry><entry s=\"DOC3/TOC3\"><text>BAZ</text><entry s=\"DOC3.1/TOC3.1\"><text>BASIL</text></entry></entry></toc>"
				+ EXPECTED_ISBN + "</title>";
	...
	public void testAlternativeId() throws Exception {
		...
		String expectedDocs = EXPECTED_ALT_ID_ENTRIES
				+ "<doc id=\"FAM_GUID1\" altid=\"test_FAM_GUID1\" src=\"DOC_GUID1.html\"/><doc id=\"FAM_GUID2\" altid=\"test_FAM_GUID2\" src=\"DOC_GUID2.html\"/><doc id=\"GENERATED_FAMILY_GUID\" altid=\"test_FAM_GUID3\" src=\"GENERATED_FAMILY_GUID.html\"/><doc id=\"GENERATED_DOC_GUID\" altid=\"test_GENERATED_DOC_GUID\" src=\"GENERATED_DOC_GUID.html\"/></docs>";
		String expectedToc = EXPECTED_TOC
				+ "<entry s=\"FAM_GUID1/TOC_GUID1\"><text>1</text></entry><entry s=\"FAM_GUID2/TOC_GUID2\"><text>2</text></entry><entry s=\"GENERATED_FAMILY_GUID/TOC_GUID3\"><text>3</text></entry><entry s=\"GENERATED_DOC_GUID/TOC_GUID4\"><text>4</text></entry></toc>";
		String expected = EXPECTED_START_MANIFEST_PREFIX + lastupdated + EXPECTED_START_MANIFEST_SUFFIX
				+ onlineexpiration + "\">" + EXPECTED_FEATURES + EXPECTED_MATERIAL_ID + EXPECTED_ARTWORK
