...
public class InitializeTaskTest {
	...
	public void shouldCreateAndSetWorkDir() throws Exception {
		...
		File workDir = new File(temporaryFolder.getRoot(), "env/data/" + dateStr + "/titleId/1");
		// when
		step.executeStep(contribution, chunkContext);
		// then
	...
	private void givenAll() {
		...
		given(chunkContext.getStepContext().getStepExecution().getJobExecution().getExecutionContext()).willReturn(jobExecutionContext);
		given(chunkContext.getStepContext().getStepExecution().getJobExecution().getJobParameters().getLong(JobParameterKey.BOOK_DEFINITION_ID)).willReturn(1l);
