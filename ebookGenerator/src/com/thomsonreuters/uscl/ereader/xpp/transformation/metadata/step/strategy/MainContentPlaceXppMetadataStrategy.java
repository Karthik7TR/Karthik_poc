...
@Component
public class MainContentPlaceXppMetadataStrategy extends AbstractPlaceXppMetadataTransformStrategy {
    private final File xslTransformationFile;
    private final XppFormatFileSystem xppFormatFileSystem;

    @Autowired
    public MainContentPlaceXppMetadataStrategy(
        ...
        @Value("${xpp.add.metadata.to.maincontent.xsl}") final File xslTransformationFile) {
        super(xslTransformationService, transformerBuilderFactory, new HashSet<>(Arrays.asList(BundleFileType.MAIN_CONTENT)));
    ...
    protected List<TransformationCommand> getTransformationCommands(
        ...
        final TransformationCommand addMetadataToMainContentCommand =
            new TransformationCommandBuilder(transformer, outputFile).withInput(inputFile).build();
        return asList(addMetadataToMainContentCommand);
    }
}
