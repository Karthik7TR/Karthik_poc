...
public class EBookRequestQueuePoller {
	...
	public void pollMessageQueue() {
		try {
			...
			if (outage == null) {
				...
				int coreThreadPoolSize = threadPoolTaskExecutor.getCorePoolSize();
				...
				if (activeThreads < coreThreadPoolSize) {
					...
					if (StringUtils.isNotEmpty(request)) {
						...
						builder.addParameter(EBookRequest.KEY_JOB_NAME, new JobParameter(EBookRequest.JOB_NAME_PROCESS_BUNDLE));
						runJob(EBookRequest.JOB_NAME_PROCESS_BUNDLE, builder.toJobParameters());
					} else {
						// log.info("No bundle requests in the message queue.");
					}
				} else {
					log.debug(String.format("There are %d active bundle processing threads running, the maximum allowed is %d.  "
							+ "No new jobs will be started until the active count is less than the maximum.", activeThreads,
							coreThreadPoolSize));
				}
			} else {
	...
	@Required
	public void setOutageProcessor(OutageProcessor outageProcessor) {
