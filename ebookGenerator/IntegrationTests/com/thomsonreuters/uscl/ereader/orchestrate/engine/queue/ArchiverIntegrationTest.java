...
public class ArchiverIntegrationTest {
	private static String message = "<eBookRequest version='1.0'><messageId>%s</messageId><bundleHash>9fef91b329e827f0af4bdf0f604c909d</bundleHash><dateTime>2016-02-10T19:42:03.522Z</dateTime><srcFile>C:/apps/titleName.gz</srcFile></eBookRequest>";
	...
	public void testRun() throws InterruptedException {
		JMSClient jmsClient = new JmsClientImpl();
		
		String messageUuid = uuidWithoutDashes();
...
//	}
	...
	private void validate(String mesageUuid) {
		validateRetrieveBundleTask(mesageUuid);
	}

	private void validateRetrieveBundleTask(String mesageUuid) {
		try {
			EBookRequest dup = bundleArchiveDao.findByRequestId(mesageUuid);
			assertNotNull(dup);
		} catch (Exception e) {
			e.printStackTrace();
			fail();
	...
	private void waitTillJobDone() throws InterruptedException {
		ThreadPoolExecutor executor = springBatchBundleTaskExecutor.getThreadPoolExecutor();
		executor.shutdown();
