...
public class ManagerDaoImpl implements ManagerDao {
	...
	public int archiveAndDeleteSpringBatchJobRecordsBefore(Date deleteBefore) {
		...
		if (jobExecutionIds.size() > 0) { // if no executions, there will be no
			...
			List<Long> jobInstanceIds = jdbcTemplate
					.queryForList("select unique job_instance_id from batch_job_execution where ("
							+ TIME_COMPARISON_COLUMN_NAME + " < ?)", args, argTypes, Long.class);

			// STEPS to be cleaned up, fetching them exec id, by exec id
			for (Long jobExecutionId : jobExecutionIds) {
				List<Long> stepExecutionIdsForJobExecutionId = jdbcTemplate.queryForList(String.format(
						"select step_execution_id from batch_step_execution where (job_execution_id = %d)",
						jobExecutionId), Long.class);
			...
			deleteSpringBatchRecords("delete from BATCH_JOB_EXECUTION_PARAMS where (JOB_EXECUTION_ID = %d)",
					jobExecutionIds);
			deleteSpringBatchRecords("delete from BATCH_JOB_EXECUTION_CONTEXT where (JOB_EXECUTION_ID = %d)",
					jobExecutionIds);
			deleteSpringBatchRecords("delete from BATCH_STEP_EXECUTION_CONTEXT where (STEP_EXECUTION_ID = %d)",
					stepExecutionIds);
		...
		try {
			...
			for (Object[] arr : objectList) {
				...
				try {
					...
					log.error("Encountered a version which is not a valid number in document metadata for book "
							+ arr[1].toString() + " job " + arr[0].toString() + " version " + arr[2].toString());
					continue;
				}
				...
				if (prevBookId == bookId && prevMajorVersion == majorVersion
						&& (!status.equals("Publish Step Completed") || maxPublishedCntr >= maxVersionKept)) {
					// Add to list to be deleted
					jobInstanceIds.add(jobId);
					log.debug("Delete book " + bookId + " jobId " + jobId + " major version " + majorVersion
							+ " status " + status);
				} else {
	...
	private void archiveJobStepDataToJobHistoryTable(List<Long> stepExecutionIds) {
		...
		for (Long stepId : stepExecutionIds) {
			try {
				...
				log.error(String.format("Error archiving job step ID %d to the JOB_HISTORY table - %s", stepId,
						e.getMessage()));
			}
		}
	}
	...
	private void deleteSpringBatchRecords(String sqlTemplate, List<Long> ids) {
		for (Long id : ids) {
			...
			try {
				...
				log.error(String.format("Error deleting old Spring Batch job record using SQL: %s - %s", sql,
						e.getMessage()));
			}
		}
	}
