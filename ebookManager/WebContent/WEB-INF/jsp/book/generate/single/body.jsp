...
<% String CUSTOM_VERSION = GenerateBookForm.Version.CUSTOM_VERSION.toString(); %>
...
<script type="text/javascript">
	...
	function changeNewVersion(versionTypeSelection){
		...
		switch (newVersionType) {
			...
			case VERSION_TYPE.MAJOR:
				...
				break;
			case VERSION_TYPE.CUSTOM_VERSION:
				newVersion = "${newCustomVersionNumber}";
				break;
	...
	function checkVersion(){
		...
		var newCustomVersion = document.getElementById('newCustomVersionBox').value;
		const versionType = $('#jsVersionTypeSelect option:selected').val();
		let message = "Generate with new version number: ";

		if (versionType === VERSION_TYPE.CUSTOM_VERSION){
			message += newCustomVersion;
		} else {
			message += newVersion;
		}

		if (newVersion == "" && newCustomVersion == "" ){
			alert("Cannot generate book: Version or new Custom Version must be selected.");
			confirmed = false;
		}
		else{
			confirmed = confirm(message);
	...
	$("#jsVersionTypeSelect").change(function()
	{
		currentVersionType= $(this).val();
	});
	$(document).ready(function(){
		document.getElementById("customVersion").style.display = "none";

	});

	function versionSelectionChange(){
		const myselect = document.getElementById("jsVersionTypeSelect");
		if(myselect.options[myselect.selectedIndex].value == "CUSTOM_VERSION") {
		...
		<form:form action="<%=WebConstants.MVC_BOOK_SINGLE_GENERATE_PREVIEW%>"
			...
			<table>
				...
				<tr>
					...
					<td id="customVersion">
						<form:input  path="newCustomVersion" id="newCustomVersionBox"/>
					</td>
					...
					<td>
						<c:choose>
							<c:when test="${!isPublished}">
								<form:select path="newVersion"  id="jsVersionTypeSelect" onchange="versionSelectionChange()">
									<form:option label="<%= MAJOR %>" value="<%= MAJOR %>" />
									<form:option label="<%= CUSTOM_VERSION %>" value="<%= CUSTOM_VERSION %>" />
								</form:select>
							</c:when>
							<c:otherwise>
								<form:select path="newVersion" id="jsVersionTypeSelect" onchange="versionSelectionChange()">
