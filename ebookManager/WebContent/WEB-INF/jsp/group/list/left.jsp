...
$(document).ready(function() {
	...
	$("form input").keyup(function(event) {
		if (event.keyCode == 13) {
			submitAuditFilterForm('<%=GroupListFilterForm.FilterCommand.SEARCH%>');
		}
	});
...
function submitGroupFilterForm(command) {
	$("#filterCommand").val(command);  // Set the form hidden field value for the operation discriminator
	$("#<%=GroupListFilterForm.FORM_NAME%>").submit();	// POST the HTML form
}
</script>

<div class="header">Filters</div>
<form:form action="<%=WebConstants.MVC_BOOK_GROUP_LIST_FILTER_POST %>"
		   commandName="<%=GroupListFilterForm.FORM_NAME%>" method="post">
	...
	<input id="bookGroupSearchButton" type="button" value="Search" onclick="submitGroupFilterForm('<%=GroupListFilterForm.FilterCommand.SEARCH%>')"/>
	<input type="button" value="Reset" onclick="submitGroupFilterForm('<%=GroupListFilterForm.FilterCommand.RESET%>')"/>
</form:form>
