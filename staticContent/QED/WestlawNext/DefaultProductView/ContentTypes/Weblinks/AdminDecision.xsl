...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
  ...
  <!-- Date.Block - Overridden here to insert the ALJ name for enhancement 446607-->
	...
	<!-- To allow preliminary information to be styled differently(ie. centered), we need to split the document
	     at the last occurance of the front.matter element -->
	...
	<xsl:template match="para" priority="1">
		...
		<xsl:choose>
			<xsl:when test="$collection = 'w_adm_fgatt' or $collection = 'w_adm_north' or $collection = 'w_adm_ust1' or $collection = 'w_adm_ust1k' or $collection = 'w_adm_ust2' or $collection = 'w_adm_canada'  or $collection = 'w_3rd_tif'">
				<xsl:choose>
					<xsl:when test="parent::para">
						<xsl:variable name="indent" select="substring(@ampexmnem,4,1)"/>
						<div>
		...
		<xsl:template match="para[contains('|w_adm_naicpro|w_adm_naicwp|w_adm_fgccab|' , concat('|', /Document/document-data/collection, '|'))]" priority="5">
		<xsl:choose>
			<xsl:when test="parent::para">
	...
	<!-- overriding the footnote reference in the Footnotes.xsl to handle different footnote markup, this was done for
	     collection 'w_adm_fgccab' only, the other rest of the template is similar to the one in Footnotes.xsl -->
		...
		<xsl:if test="string-length($refNumberText) &gt; 0">
			<xsl:variable name="contents">
				...
				<span>
					...
					<xsl:choose>
						<xsl:when test="not(/*[1]/self::summary or /*[1]/self::summaries) and string-length($footnoteId) &gt; 0 and (/Document//footnote.reference/@ID=$footnoteId or /Document//footnote.reference/@refid=$footnoteId) ">
							<xsl:choose>
						...
						<xsl:when test="string-length(key('distinctFootnoteAnchorReferenceRefIds', $footnoteId)) &gt; 0 and /Document/document-data/collection = 'w_adm_fgccab'">
							<a href="#co_footnoteReference_{$footnoteId}">
								<xsl:value-of select="$refNumberText"/>
							</a>
						</xsl:when>
