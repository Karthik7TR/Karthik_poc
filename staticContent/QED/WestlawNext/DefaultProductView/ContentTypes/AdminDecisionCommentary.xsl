...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:template match="decision" name="decisionOrig">
	...
	<xsl:template match="head[following-sibling::para]">
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<!--Don't wrap with div.co_center-->
				<xsl:call-template name="head"/>
			</xsl:when>
			<xsl:otherwise>
				<!--Render original non enhanced markup. -->
				<div class="&centerClass;">
					<xsl:call-template name="head"/>
				</div>
			</xsl:otherwise>
	...
	<xsl:template match="main.text//head">
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<!--Do left alignment in enhancement mode -->
				<xsl:choose>
	...
	<!-- Citation should display at the top in bold, followed by an extra line space. -->
	<xsl:template match="*[md.cites]">
		<xsl:variable name="contents">
			<xsl:apply-templates/>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode and string-length($contents) &gt; 0">
				<!-- To apply enhancement styles cites should be wrapped with div -->
				<div>
					<xsl:copy-of select="$contents"/>
				</div>
			</xsl:when>
	...
	<xsl:template match="decision" priority="1">
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<xsl:if test="(prelim.block[front.matter] and count(prelim.block/node()[following-sibling::front.matter]) &gt; 0) or (not(prelim.block[front.matter]) and count(node()[following-sibling::front.matter]) &gt; 0)">
					<div class="&preFrontMatterClass;">
			...
			<xsl:otherwise>
				...
				<xsl:call-template name="decisionOrig"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="author.block" priority="1">
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<!-- Suppress to display authors before document title. -->
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="author.block" mode="Enhancement">
		<xsl:call-template name="author"/>
	</xsl:template>

	<xsl:template match="front.matter[date.block]" name="frontMatterBase" priority="1">
		<xsl:param name="suppressHeadComment" select="false()"/>
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<!-- Consider all nodes except document title as preliminary content -->
				<div class="&propBlockClass;">
					<xsl:choose>
						<xsl:when test="source and date.block">
							<div class="&publicationLineClass;">
								<!--Renders publication title-->
								<xsl:apply-templates select="source[following-sibling::date.block and not(preceding-sibling::source)]"/>
							...
							<xsl:apply-templates select="node()[not(self::title.block or (self::source and following-sibling::date.block and not(preceding-sibling::source)) or self::date.block)]"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:apply-templates select="node()[not(self::title.block)]"/>
						</xsl:otherwise>
					</xsl:choose>
						<!-- Display authors before document title. -->
					<xsl:if test="not(following-sibling::front.matter)">
						<xsl:apply-templates select="//author.block" mode="Enhancement"/>
					</xsl:if>
				</div>
				
				<!-- preliminary content should be followed by a horizontal line separator, followed by the document title in large font and bold. -->
				<xsl:if test="title.block">
					<div class="&dividerClass;"></div>
					<xsl:call-template name="wrapContentBlockWithCobaltClass">
						<xsl:with-param name="contents">
							<xsl:apply-templates select="title.block" mode="Enhancement"/>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
				<xsl:if test="not($suppressHeadComment)">
					<xsl:comment>&EndOfDocumentHead;</xsl:comment>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<!--Renders original non enhanced markup. -->
				<xsl:call-template name="frontMatterWithDateBlockOrig"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template match="title.block" mode="Enhancement">
		<xsl:choose>
			<!-- Make document title in large font and bold -->
	...
	<xsl:template match="prelim.synopsis.body/head[not(headtext)]">
		<xsl:choose>
			<xsl:when test="$IsCommentaryEnhancementMode">
				<!--Don't wrap with h2-->
				<xsl:apply-templates/>
