...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:template match="Document">
		<div id="&documentId;">
			<xsl:call-template name="AddDocumentClasses">
				<xsl:with-param name="contentType" select="'&contentTypeCaselawAMCClass;'"/>
			</xsl:call-template>
	...
	<xsl:variable name="firstAdjustedParallelCite" select="/Document/n-metadata/metadata.block/md.identifiers/md.cites/md.parallelcite/md.parallelcite.info/md.adj.display.parallelcite"/>
	...
	<xsl:template match="md.cites" mode="top">
		<div class="&citesClass;">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="string-length($firstAdjustedParallelCite) &gt; 0">
							<xsl:apply-templates select="$firstAdjustedParallelCite" />
						</xsl:when>
	...
	<!-- amc headnote -->
	<xsl:template match="amc.headnote.block">
		<div class="&headnoteBlockClass;">
			<xsl:apply-templates />
		</div>
	</xsl:template>

	<xsl:template match="amc.headnote">
		<div class="&headnoteClass;">
			<xsl:apply-templates />
		</div>
	</xsl:template>

	<xsl:template match="amc.headnote.body">
		<div>
			<xsl:apply-templates />
		</div>
	</xsl:template>

	<xsl:template match="amc.key.line">
		<div>
			<xsl:apply-templates />
		</div>
	</xsl:template>

	<xsl:template match="text.omission">
