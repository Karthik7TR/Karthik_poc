  ...
  <xsl:template match="Document">
        ...
        <xsl:choose>
          <xsl:when test="$IsCommentaryEnhancementMode">
            <xsl:value-of select="' &commentaryDocumentEnhancementClass; &contentTypeAnalyticalEaganProductClass;'"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="' &contentTypeAnalyticalEaganProductClass;'"/>
          </xsl:otherwise>
        </xsl:choose>
			</xsl:with-param>
