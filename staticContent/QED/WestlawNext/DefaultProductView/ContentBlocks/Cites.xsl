	...
	<xsl:template match="cmd.cites">
		...
		<xsl:if test="string-length($contents) &gt; 0">
			<div class="&citesClass;">
				...
				<xsl:if test="contains($contents, '26 U') and cmd.alternative.cites/cmd.alternative.cite">
					<xsl:variable name="altcite" select="cmd.alternative.cites/cmd.alternative.cite[contains(text(), 'I.R.C.')]"/>
					<xsl:if test="$altcite">
						<xsl:value-of select = "concat(', ', ($altcite)[1]/text())" />
					</xsl:if>
				</xsl:if>
			</div>
		</xsl:if>
