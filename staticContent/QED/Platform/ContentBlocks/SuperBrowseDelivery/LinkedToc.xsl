...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:variable name="documentDataTitle" select="../../document-data/title" />
	...
	<xsl:variable name="displayCurrentness">
		...
		<xsl:if test="prelim.block or content.metadata.block or doc.title">
				<div class="&documentHeadClass;">
					...
					<xsl:choose>
						<xsl:when test="doc.title | prelim.block/doc.title">
							<xsl:apply-templates select="doc.title | prelim.block/doc.title" mode="superBrowseHeader"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:apply-templates select="../../document-data/title"  mode="superBrowseHeader"/>
						</xsl:otherwise>
					</xsl:choose>
				</div>
	...
	<xsl:template match="title" mode="superBrowseHeader">
		<div>
			<xsl:attribute name="class">
				<xsl:text>&simpleContentBlockClass;</xsl:text>
				<xsl:text><![CDATA[ ]]>&alignHorizontalLeftClass;</xsl:text>
				<xsl:text><![CDATA[ ]]>&boldClass;</xsl:text>
			</xsl:attribute>
			<xsl:call-template name="getDocumentLink"/>
		</div>
	</xsl:template>
	
	<xsl:template match="doc.title" mode="superBrowseHeader">
		<xsl:variable name="documentLinkHtml">
			<xsl:call-template name="getDocumentLink"/>
		</xsl:variable>
		<xsl:call-template name="wrapContentBlockWithAdditionalCobaltClasses">
			...
			<xsl:with-param name="contents" select="$documentLinkHtml"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="getDocumentLink">
		<xsl:choose>
			<xsl:when test="ancestor::Document/toc-data">
				<xsl:variable name="documentAnchor" select="ancestor::Document/toc-data//ul/li[@class = 'co_active']/a" />
				<xsl:if test="$documentAnchor">
					<a><xsl:attribute name="href"><xsl:value-of select="$documentAnchor/@href"/></xsl:attribute><xsl:value-of select="normalize-space(.)"/></a>
				</xsl:if>
			</xsl:when>
