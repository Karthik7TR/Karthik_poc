      ...
      <xsl:choose>
        <xsl:when test="$imageMetaData/md.image.renderType = 'BlobLink'">
          <xsl:call-template name="createDocumentBlobLink">
            ...
            <xsl:with-param name="data-language">
              <xsl:if test="parent::attached.file/@language">
                <xsl:value-of select="parent::attached.file/@language"/>
              </xsl:if>
            </xsl:with-param>
          </xsl:call-template>
        </xsl:when>
        <xsl:when test="$imageMetaData/md.image.renderType = 'XlsLink' or $imageMetaData/md.image.renderType = 'XlsxLink'">
          <xsl:variable name="sheetMimeType">
            <xsl:choose>
          ...
          <xsl:call-template name="createDocumentBlobLink">
            <xsl:with-param name="guid" select="@tuuid"/>
            <xsl:with-param name="targetType" select="@ttype"/>
            <xsl:with-param name="mimeType" select="$sheetMimeType"/>
            <xsl:with-param name="contents">
              <xsl:apply-templates />
  ...
  <xsl:template name="createDocumentBlobLink">
    ...
    <xsl:if test="string-length($contents) &gt; 0">
      ...
      <a>
        ...
        <xsl:if test="string-length($data-language) &gt; 0">
            ...
            <xsl:attribute name="class">
              <xsl:choose>
                <xsl:when test="$mimeType = 'application/msword'">
                  <xsl:value-of select="'&wordIconClass;'"/>
                </xsl:when>
                <xsl:when test="$mimeType = '&xlsMimeType;' or $mimeType = '&xlsxMimeType;'">
                  <xsl:value-of select="'&xlsIconClass;'"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="$displayIconClassName"/>
                </xsl:otherwise>
