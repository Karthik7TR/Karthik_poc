...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
  ...
  <xsl:template name="FooterCitation">
    <xsl:variable name="citation">
      <xsl:choose>
        <xsl:when test="/Document/document-data/collection = 'w_3rd_corpdir'">
  ...
  <xsl:template match = "content.metadata.block[1]">
    <xsl:if test="cmd.display.primarycite" />
  </xsl:template>

	<xsl:template match="directory.info/name">
		<xsl:choose>
	...
	<xsl:template match="directory.info/*[name() != 'starpage.anchor' and name() != 'name']">
		<div>
			<xsl:apply-templates />
		</div>
	</xsl:template>

	<xsl:template match="directory.info/address/*[name() != 'starpage.anchor']">
		<div>
			<xsl:apply-templates />
		</div>
	</xsl:template>
	...
	<xsl:template match="message.block/copyright.line">
		<xsl:apply-templates />
	</xsl:template>
  
  <!--Suppress the normal "cmd.cites" elements -->
  <xsl:template match="cmd.cites" priority="5"/>
  
  
  <xsl:template name="headerCitation" match="md.cites" priority="5">
