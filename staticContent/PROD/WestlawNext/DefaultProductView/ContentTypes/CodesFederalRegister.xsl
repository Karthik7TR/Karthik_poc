...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:template match="md.cites" priority="3">
		<xsl:variable name="displayableCites" select="md.parallelcite/md.fr.cites/md.fr.cite/md.fr.display.cite[./@display = 'Y'] " />
		<xsl:variable name="displayableTypeAbbr" select="md.parallelcite/md.fr.cites/md.fr.cite/md.fr.type.abbr" />
		...
		<xsl:choose>
			<xsl:when test="string-length($displayableCites) &gt; 0">
				<div class="&citesClass;">
					...
					<xsl:if test="string-length($displayableTypeAbbr) &gt; 0">
						<xsl:value-of select="$displayableTypeAbbr"/>
					</xsl:if>
				</div>
			</xsl:when>
			<xsl:otherwise>
				<xsl:if test="string-length($firstLineCite) &gt; 0">
					<div class="&citesClass;">
	...
	<xsl:template match="md.cites[/Document/document-data/collection = 'w_codesfrprespnvdp']" priority="2" >
	...
	<!-- Suppress displaying of Citation-->
	<xsl:template match="cmd.cites" priority="3" />
	...
	<!-- Special handling of star page without pageset information -->
	<xsl:template match="starpage.anchor[not(@pageset) and @ID]" priority="3">
		<xsl:call-template name="displayStarPage">
			<xsl:with-param name="starPageText">
				<xsl:apply-templates />
			</xsl:with-param>
			<xsl:with-param name="numberOfStars" select="1" />
			<xsl:with-param name="id" select="concat('&pinpointIdPrefix;', @ID)" />
		</xsl:call-template>
