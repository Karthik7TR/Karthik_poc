...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:variable name="content-metadata-block" select="/Document/map/entry[key='novus']/value/n-metadata/metadata.block" />
	<xsl:variable name="primarycite" select="$content-metadata-block/md.identifiers/md.cites/md.primarycite/text()" />

	<xsl:variable name="infoType" select="/Document/n-metadata//md.infotype"/>
	<xsl:variable name="aoaInfoType" select="'hk-legis-AOA'"/>
	<xsl:variable name="legislocInfoType" select="'hk-legisloc'"/>
	...
	<xsl:template match="Document">
		<div id="&documentClass;">
			...
			<xsl:choose>
				<xsl:when test="$infoType = $aoaInfoType">
					<div class="&centerClass;">
						...
						<xsl:apply-templates select="n-metadata/metadata.block/md.descriptions/md.longtitle"/>
						<!--<xsl:apply-templates select="n-metadata/metadata.block/md.dates/"></xsl:apply-templates>-->
					</div>
					<xsl:comment>&EndOfDocumentHead;</xsl:comment>

					<xsl:apply-templates select="n-docbody/document/provisions-table"/>
					<xsl:apply-templates select="n-docbody/document/copyright-message"/>
				</xsl:when>

				<xsl:when test="$infoType = $legislocInfoType">
					<div class="&centerClass;">
						<xsl:apply-templates select="n-metadata/metadata.block/md.identifiers/md.cites/md.primarycite" />
						<xsl:apply-templates select="n-metadata/metadata.block/md.descriptions/md.title" />
						<xsl:apply-templates select="n-metadata/metadata.block/md.descriptions/md.longtitle"/>
