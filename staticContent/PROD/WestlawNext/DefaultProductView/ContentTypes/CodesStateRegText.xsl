...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:template match="reg.front/prelim.block" priority="2" >
		<div class="&simpleContentBlockClass; &prelimBlockClass;">
			<div class="&centerClass;">
                ...
                <xsl:apply-templates select="/Document/n-metadata/metadata.block/md.dates/md.starteffective"/>
                <xsl:apply-templates select="prelim.head[4]"/>
                <xsl:apply-templates select="prelim.head[5]"/>
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template match="md.starteffective">
		<xsl:if test="number(.) = .">
			...
			<xsl:call-template name="renderHeadTextDiv">
				<xsl:with-param name="contents" select="concat('&srEffectiveDate;',$formattedDate)"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="content.metadata.block/cmd.identifiers/cmd.cites" priority="2">
		<div class="&citesClass; &titleClass;">
			<xsl:choose>
				<xsl:when test="cmd.first.line.cite and cmd.second.line.cite">
