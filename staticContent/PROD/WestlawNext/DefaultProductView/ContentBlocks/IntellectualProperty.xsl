...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:CiteQuery="xalan://com.thomsonreuters.uscl.ereader.format.links.CiteQueryAdapter" xmlns:UrlBuilder="xalan://com.thomsonreuters.uscl.ereader.format.links.UrlBuilderAdapter" xmlns:DocumentExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.DocumentExtensionAdapter" xmlns:CitationLinkExtension="xalan://com.thomsonreuters.uscl.ereader.format.text.CitationLinkExtensionAdapter" extension-element-prefixes="DocumentExtension CiteQuery UrlBuilder CitationLinkExtension">
	...
	<xsl:template name="join">
		...
		<xsl:for-each select="$nodes">
			<xsl:choose>
				...
				<xsl:when test="substring(name(.), string-length(name(.)) - 4) = '.main' or name(.) = 'label'">
					<strong>
						<xsl:apply-templates />
					</strong>
				</xsl:when>
	...
	<xsl:template match="claims.block/*[not(self::head)]">
		<xsl:call-template name="wrapWithDiv">
	...
	<xsl:template match="specification">
		<xsl:call-template name="wrapWithDiv">
			<xsl:with-param name="class" select="'co_specification'" />
		</xsl:call-template>
	</xsl:template>

	<xsl:template match="specification/*[not(self::head)]">
		<xsl:call-template name="wrapWithDiv">
			<xsl:with-param name="class" select="'&panelBlockClass;'" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="md.ip.image.link | md.gateway.image.link" name="createIPImageLink">
