...
public final class PublishingStatsServiceTest
{
    ...
    {
        final Long ebookDefId = Long.valueOf(1);

        EasyMock.expect(mockDao.getMaxGroupVersionById(ebookDefId)).andReturn(Long.valueOf(10));
        EasyMock.replay(mockDao);
        final Long maxGroupVersion = service.getMaxGroupVersionById(ebookDefId);
        Assert.assertEquals(Long.valueOf(10), maxGroupVersion);
        EasyMock.verify(mockDao);
    }
    ...
    {
        final Long ebookDefId = Long.valueOf(1);
        final List<String> subGroupHeadings = new ArrayList<>();

        EasyMock.expect(mockDao.findSuccessfullyPublishedsubGroupById(ebookDefId)).andReturn(subGroupHeadings);
        EasyMock.replay(mockDao);
        final boolean changed = service.hasSubGroupChanged("subGroupHeading", ebookDefId);
