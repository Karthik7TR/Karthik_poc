...
public class BookDefinitionLockDaoTest  {
	private final static List<BookDefinitionLock> BOOK_LOCK_LIST = new ArrayList<BookDefinitionLock>();
	...
	public void testFindAllActiveLocks() {
		...
		EasyMock.expect(mockCriteria.add(Restrictions.ge("checkoutTimestamp", EasyMock.anyObject(Date.class)))).andReturn(mockCriteria);
		EasyMock.expect(mockCriteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)).andReturn(mockCriteria);
		...
		List<BookDefinitionLock> locks = dao.findAllActiveLocks();
	...
	public void testFindLocksByBookDefinition() {
		...
		EasyMock.expect(mockSessionFactory.getCurrentSession()).andReturn(mockSession);
		EasyMock.expect(mockSession.createCriteria(BookDefinitionLock.class)).andReturn(mockCriteria);
		EasyMock.expect(mockCriteria.add(Restrictions.eq("ebookDefinition", EasyMock.anyObject(BookDefinition.class)))).andReturn(mockCriteria);
		EasyMock.expect(mockCriteria.addOrder(EasyMock.anyObject(Order.class))).andReturn(mockCriteria);
		EasyMock.expect(mockCriteria.list()).andReturn(BOOK_LOCK_LIST);
		EasyMock.replay(mockSessionFactory);
		EasyMock.replay(mockSession);
	...
	public void testFindBookDefinitionLockByPrimaryKey() {
		EasyMock.expect(mockSessionFactory.getCurrentSession()).andReturn(mockSession);
		EasyMock.expect(mockSession.createCriteria(BookDefinitionLock.class)).andReturn(mockCriteria);
		EasyMock.expect(mockCriteria.add(Restrictions.eq("ebookDefinitionLockId", EasyMock.anyObject(Long.class)))).andReturn(mockCriteria);
		...
		BookDefinitionLock lock = dao.findBookDefinitionLockByPrimaryKey(1L);
