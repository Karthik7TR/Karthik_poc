...
public class LibraryListDaoImpl implements LibraryListDao {
	...
	public List<LibraryList> findBookDefinitions(String sortProperty, boolean isAscending, int pageNumber, int itemsPerPage) {
		...
		if(sortProperty.equalsIgnoreCase("isDeletedFlag")) {
			...
			sql.append(String.format("order by %s %s", sortProperty, direction));
		}
...
log.debug("SQL: " + sql.toString());
	...
	public Long countNumberOfBookDefinitions() {
		return (Long) jdbcTemplate.queryForLong("SELECT COUNT (book.EBOOK_DEFINITION_ID) FROM EBOOK_DEFINITION book");
	}
...
class LibraryListRowMapper implements RowMapper<LibraryList> {
	...
	public LibraryList mapRow(ResultSet resultSet, int rowNum) throws SQLException {
